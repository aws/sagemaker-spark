...
/**
  * Extracts a features column from a Row and serializes as a LibSVM record. Each serialized
  * LibSVM record has a label of 0.
  *
  ...
  */
class UnlabeledLibSVMRequestRowSerializer (val schema : Option[StructType] = None,
  ...
  override def serializeRow(row: Row): Array[Byte] = {
    LibSVMRequestRowSerializerUtils.serializeLabeledFeatureVector(0.0,
      row.getAs[Vector](featuresFieldIndex))
  }
